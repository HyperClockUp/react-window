...
export class Application extends React.PureComponent<IProps, IState> {
    ...
    getHeaderButtons: Function = (): Array<JSX.Element> => {
        ...
        if (this.state.windowAction.includes("close")) {
            buttons.push(
                <button
                    className="close"
                    key="close"
                    onClick={this.state.action.close}>
                    ×
                </button>
            )
        }
    ...
    handleMouseMove = (event: MouseEvent): any => {
        ...
        let curX: number = event.clientX
        let curY: number = event.clientY
