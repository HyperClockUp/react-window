...
import ContextMenuHandle from './src/context/menu';
import './Windows.scss';
...
class Windows extends React.PureComponent<IProps, IState> {
  ...
  componentWillMount = () => {
    ...
    fetch('https://api.sumt.cn/api/bing.php?idx=0&format=json')
      .then(res => res.json())
      .then(json => {
        this.setState({
          bg: `${json.img_url}`
  ...
  updateWindow = (origin: string, log: boolean): void => {
    this.setState(
      {
        appList: windowsController.appList,
        appOpenList: windowsController.appOpenList,
  ...
  createIconList = (): Array<Type.IIcon> => {
    return this.state.appList.map((item, index) => {
      const config: Type.IApp = item.getConfig();
      const tempIcon: Type.IIcon = {
        app: config.title,
        action: {
          open: () => {}
        },
  ...
  createTaskList = (): Array<Type.ITask> => {
    return this.state.applicationStateList.map((item, index) => {
      const tempTask: Type.ITask = {
        app: item.app,
        action: {
          close: () => {
          ...
          normal: () => {
            const tempArr = this.state.applicationStateList.slice(0);
            for (const appState of tempArr) {
      ...
      return tempArr.map((item, index) => {
        return (
          <Application
            windowInfo={item}
            key={item.title}
            body={this.state.appOpenList[index].getComponent()}
          />
        );
      });
    } else {
  ...
  createContextMenu: Function = (e: MouseEvent, context: Type.IContext) => {
    const operations: Array<Type.IContextItem> = [
      {
    ...
    const contextList: Array<JSX.Element> = operations.map((item, index) => {
      return (
        <div
        ...
        >
          {(() => {
            if (item.icon) {
              return <img className="img" src={item.icon} alt={item.title} />;
            }
          })()}
      ...
      <ContextMenuHandle.Provider value={this.createContextMenu}>
        ...
        >
          ...
          <Desktop
            ...
            openApplication={windowsController.openApp}
            bgUrl={this.state.bg}
          />
